...
function decodeUnixTimestamp(t) {
  ...
  function decodeTeleGrid(s, es) {
    let top = s.substring(0,9)
    let aTop = es.substring(0,9)
    let mid = s.substring(9,18)
    let aMid = es.substring(9,18)
    let ground = s.substring(18)
    let aGround = es.substring(18)
    // console.log(ground)
    // console.log(mid)
    // console.log(top)
    let score = 0
    ...
    for (let i = 0; i < 9; i ++) {
      if (ground.substring(i,i+1) != aGround.substring(i,i+1)) {
        if (ground.substring(i, i+1) != "0") {score += 2}
      }if (mid.substring(i,i+1) != aMid.substring(i,i+1)) {
        if (mid.substring(i, i+1) != "0") {score += 3}
      }if (top.substring(i,i+1) != aTop.substring(i,i+1)) {
...
// auto charge = 8?
  ...
  function topScores(g){
    let num = 0
    for (let i = 0; i < 9; i++) {
      if (g.substring(i,i+1) != "0") {num ++}
    } return num
  }

  function midScores(g){
    let num = 0
    for (let i = 9; i < 18; i++) {
      if (g.substring(i,i+1) != "0") {num ++}
    } return num
