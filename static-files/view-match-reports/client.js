    ...
    success: (data) => {
      ...
      $.each(data, function (i, item) {
        ...
        var newRow =
          ...
          "</td><td>" +
          decodeTeleGrid(item.grid, item.auto_grid) +
          "</td><td>" +
      ...
      $.each(teams, function (i, team) {
        ...
        const avgGrid =
          Math.round(
            (filtered.reduce(
              (total, next) =>
                total + parseInt(decodeTeleGrid(next.grid, next.auto_grid)),
              0
            ) /
              filtered.length) *
              100
        ...
        const aggressionNum =
          ...
          Math.round(
            (filtered.reduce(
              (total, next) => total + parseInt(topScores(next.grid)),
              0
            ) /
              filtered.length) *
              100
          ...
          Math.round(
            (filtered.reduce(
              (total, next) => total + parseInt(midScores(next.grid)),
              0
            ) /
              filtered.length) *
              100
          ...
          Math.round(
            (filtered.reduce(
              (total, next) => total + parseInt(floorScores(next.grid)),
              0
            ) /
              filtered.length) *
              100
